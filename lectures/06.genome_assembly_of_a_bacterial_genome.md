...
Before proceeding with genome assembly, 
it is important to check if the expected species 
or strains can be identified in the data or if there is any contamination.

As this step is not specific to genome assembly, 
it is covered in a dedicated [tutorial]({{site.url}}/extras/contamination_checking).
...
<code>Shovill</code> is for isolate data only, primarily small haploid organisms. It will NOT work on metagenomes or larger genomes
</div>
...
To avoid unnecessary waiting time and server overload during the course, please retrieve the precomputed outputs from the shared history.
</div>
...
<img src="{{site.url}}/images/spades.png" width="100%">
