...
A whole-genome shotgun approach was used to produce a series of short sequence reads on an Illumina DNA 
sequencing instrument for the mutant strain.
...
1. `JBrowse` Tool with the following parameters
   ...
   the **variants found by snippy** (the `.gff` file) and the **annotated reference genome** (the `wildtype.gff`)
      ...
      - In *Track Group*:  annotated reference
        - *Insert Track Group*
        - *Track Category*:  Annotated Reference
            - *Track Type*: GFF/GFF3/BED Features
